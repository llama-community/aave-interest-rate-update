...
library AaveV2Helpers {
        ...
        returns (ReserveConfig[] memory)
    {
        ...
        for (uint256 i = 0; i < vars.reserves.length; i++) {
            ...
            ReserveTokens memory reserveTokens = _getStructReserveTokens(vars.configs[i].underlying, marketName);
    ...
    function _getConfigData(string memory marketName, address tokenAddress) internal view returns (ConfigLite memory) {
        ...
        address interestRateStrategyAddress = market.POOL.getReserveData(tokenAddress).interestRateStrategyAddress;
        return
            ConfigLite({
                decimals: decimals,
                ltv: ltv,
        ...
        returns (ReserveTokens memory)
    {
        ...
        (reserveTokens.aToken, reserveTokens.stableDebtToken, reserveTokens.variableDebtToken) = market
    ...
    function _validateReserveConfig(ReserveConfig memory expectedConfig, ReserveConfig[] memory allConfigs)
        internal
        view
    {
        ReserveConfig memory config = _findReserveConfig(allConfigs, expectedConfig.symbol, false);
        require(config.underlying == expectedConfig.underlying, "_validateEnsConfigsInAave() : INVALID_UNDERLYING");
        require(config.decimals == expectedConfig.decimals, "_validateEnsConfigsInAave: INVALID_DECIMALS");
        require(config.ltv == expectedConfig.ltv, "_validateEnsConfigsInAave: INVALID_LTV");
        require(
        ...
        require(config.isActive == expectedConfig.isActive, "_validateEnsConfigsInAave: INVALID_IS_ACTIVE");
        require(config.isFrozen == expectedConfig.isFrozen, "_validateEnsConfigsInAave: INVALID_IS_FROZEN");
    }
    ...
    function _validateInterestRateStrategy(
        ...
        require(
            strategy.EXCESS_UTILIZATION_RATE() == expectedStrategyValues.excessUtilization,
            "_validateInterestRateStrategy() : INVALID_EXCESS_RATE"
        );
        require(
            strategy.OPTIMAL_UTILIZATION_RATE() == expectedStrategyValues.optimalUtilization,
            "_validateInterestRateStrategy() : INVALID_OPTIMAL_RATE"
        );
        require(
            strategy.addressesProvider() == address(market.POOL_ADDRESSES_PROVIDER),
    ...
    function _noReservesConfigsChangesApartNewListings(
        ...
        for (uint256 i = 0; i < allConfigsBefore.length; i++) {
            ...
            require(
                allConfigsBefore[i].liquidationThreshold == allConfigsAfter[i].liquidationThreshold,
                "_noReservesConfigsChangesApartNewListings() : UNEXPECTED_LIQ_THRESHOLD_CHANGED"
            );
            require(
                allConfigsBefore[i].liquidationBonus == allConfigsAfter[i].liquidationBonus,
                "_noReservesConfigsChangesApartNewListings() : UNEXPECTED_LIQ_BONUS_CHANGED"
            );
            require(
                allConfigsBefore[i].reserveFactor == allConfigsAfter[i].reserveFactor,
                "_noReservesConfigsChangesApartNewListings() : UNEXPECTED_RESERVE_FACTOR_CHANGED"
            );
            require(
                allConfigsBefore[i].usageAsCollateralEnabled == allConfigsAfter[i].usageAsCollateralEnabled,
                "_noReservesConfigsChangesApartNewListings() : UNEXPECTED_USAGE_AS_COLLATERAL_ENABLED_CHANGED"
            );
            require(
                allConfigsBefore[i].borrowingEnabled == allConfigsAfter[i].borrowingEnabled,
                "_noReservesConfigsChangesApartNewListings() : UNEXPECTED_BORROWING_ENABLED_CHANGED"
            );

            require(
                allConfigsBefore[i].stableBorrowRateEnabled == allConfigsAfter[i].stableBorrowRateEnabled,
    ...
    function _validateReserveTokensImpls(
        ...
        require(
            IInitializableAdminUpgradeabilityProxy(config.aToken).implementation() == expectedImpls.aToken,
    ...
    function _repay(
        ...
        require(debtAfter == ((debtBefore > amount) ? debtBefore - amount : 0), "_repay() : INCONSISTENT_DEBT_AFTER");
    }
